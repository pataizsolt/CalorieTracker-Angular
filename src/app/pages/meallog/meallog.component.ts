...
/*onSubmit3(){
  let temp=this.user?.id+"@"+this.newMealLog.get("datepicker")?.value;
  this.mealLogService.getSpecific(temp).subscribe(data => {
    this.mealLogSpecific =  data.map(e =>{
      return {
        id: e.id,
        userid: e.userid,
        date: e.date,
...
onSubmit2(){
  let temp=this.user?.id+"@"+this.select.get('id')?.value;
  this.mealLogService.getSpecific(temp).subscribe(data => {
    this.mealLogSpecific =  data.map(e =>{
      return {
        id: e.id,
...
onSubmit2(){
  ...
  this.mealLogService.getById((this.user?.id+"@"+this.select.get('id')?.value)).subscribe(data => {
    this.temp = data?.date as number
    return data?.date as number;
  });


  const mealLog: MealLog = {
    id: (this.user?.id+"@"+this.select.get('id')?.value/*+Date.parse(this.newMealLog.get("datepicker")?.value)*/),
    userid: this.user?.id+"",
    date: this.temp,
